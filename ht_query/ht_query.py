...
class HTSearchQuery:
    ...
    @staticmethod
    def create_boost_phrase_fields():
        pf = [
            # phrase fields ==> Once the list of matching documents has been identified using the fq and qf parameters,
            # the pf parameter can be used to "boost" the score of documents in cases where all of the terms
    ...
    def make_solr_query(
        ...
        if self.solr_facet_filters:
            query_dict.update(HTSearchQuery.facet_creator(self.solr_facet_filters.get("facet")))
        ...
        if query_filter:
            if filter_dict:
                query_dict.update({"fq": HTSearchQuery.query_filter_creator_string("id",
                                                                                   filter_dict.get("id"))})
