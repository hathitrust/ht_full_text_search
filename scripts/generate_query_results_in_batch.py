...
def get_solr_results_without_filter_by_id(ht_full_search_obj: HTFullTextSearcher,
                                          query: dict, fl: list, solr_url: str):
    """
    ...
    for query in list_queries:
        ...
        if args.filter_path is None:
            ...
            for doc, debug_info in ht_full_search.retrieve_documents_from_file(
                    list_ids=list_ids, fl=fl, solr_url=solr_url, query_filter=query_filter,
                    query_string=query["query_string"], operator=query["operator"]):
        ...
        df.to_csv(
            path_or_buf=f'{os.getcwd()}/scripts/query_results/{query["query_fields"]}_{query["query_string"]}_{query["operator"]}_{args.env}.csv',
            index=False,
            sep="\t"
        )
