...
API is implemented to search based on additional filters for location, source, language and publication year.

![Advanced_search.png](image.png)

### Advanced Search endpoint:

Once the API is up, we can use the command below for advanced search:

`curl --location 'http://localhost:8000/advanced_search/' \
  ...
  "criteria": [
    {
      ...
      "match_type": "any of these words"
    },
    {
      "field": "Author",
      "query": "Keynes",
      "match_type": "all of these words"
    }
  ],
  "field_operators": ["AND"],
  "file_type": "json",
...
| ----------------- | --------------- | -------------------------------------------------------------------------------------------------------------- |
| `criteria`        | List of Objects | List of search conditions. Each object includes `field`, `query`, and `match_type`.                            |
| ├─ `field`        | String          | The metadata field to search in (e.g., `"title"`, `"Author"`).                                                 |
| ├─ `query`        | String          | The user-provided search text.                                                                                 |
| └─ `match_type`   | String          | Type of match to apply. Options:<br>• `"all of these words"`<br>• `"any of these words"`<br>• `"exact phrase"` |
| `field_operators` | List of Strings | Logical connectors (`"AND"`, `"OR"`) between search criteria. Must be one fewer than the number of criteria.   |
| `file_type`       | String          | Output format. Options typically include `"json"`, `"csv"`, etc.                                               |
| `start_year`      | String or Int   | Lower bound for publication year filter. Empty string (`""`) means no constraint.                              |
| `end_year`        | String or Int   | Upper bound for publication year filter. Empty string means no constraint.                                     |
| `in_year`         | String or Int   | Exact publication year to filter. Takes precedence over `start_year`/`end_year` if set.                        |
| `languages`       | List of Strings | List of preferred languages (e.g., `["English"]`).                                                             |
| `formats`         | List of Strings | List of formats to filter by (e.g., `"Audio"`, `"Biography"`).                                                 |
| `location`        | String          | Location or institution filter (e.g., `"University of Minnesota"`).                                            |

### Facet Mapping:

Each field in the input payload (e.g., "languages", "formats", "location", etc.) is mapped to the actual Solr facet fields using the `field_facet_mapping` defined in the `config_query.yaml` file.

### Solr Query Construction – `get_criteria_fields_query()`

The `get_criteria_fields_query()` function constructs the Solr `q` parameter by processing user-provided search criteria and logical field operators.

**Parameters:**

| Parameter         | Type             | Description                                                                                                    |
| ----------------- | ---------------- | -------------------------------------------------------------------------------------------------------------- |
| `criterias`       | `List[Criteria]` | A list of search conditions. Each `Criteria` object contains `field`, `query`, and `match_type`.               |
| `field_operators` | `List[str]`      | Logical connectors such as `AND` or `OR` used between each criterion.                                          |
| `config_data`     | `Dict`           | Configuration dictionary that includes `field_search_map`, which maps user input fields to actual Solr fields. |

**Returns:**  
| Return Value | Type | Description |
| -------------- | ----------- | ------------------------------------------------------------------------------- |
| `fields` | `List[str]` | List of Solr fields that were queried. |
| `joined_query` | `str` | Final `q` string for Solr, with all criteria joined by the specified operators. |

### Filter Query Construction – `build_fq_query()`

The `build_fq_query()` function is responsible for generating the Solr `fq` (filter query) clauses from user-defined filters in the advanced search request.

### Internal Calls

#### 1. `build_date_filter(start_year, end_year, in_year)`

Handles the logic for publication year filtering. It supports three filtering modes:

- **`start_year`**: Filters documents published **from this year onward**.
- **`end_year`**: Filters documents published **up to and including this year**.
- **`in_year`**: Filters documents published **exactly in this year** (overrides `start_year` and `end_year` if provided).

#### 2. `build_field_filters(field_facet_mapping, request_fields)`

Builds facet-based filters using the mappings defined in config_query.yaml. These include:
...
**Returns:**  
A Solr filter query string like:
`'fq': 'publishDateTrie:[ 1950 TO * ] AND language008_full:(English) AND format:(Audio OR Biography) AND htsource:"University of Minnesota"'
`

### CSV File Generation

If the request payload contains `"file_type": "csv"`, the system will generate a CSV file named `export.csv` under the `./csv_files` directory.

Upon successful generation, the function -`write_csv_and_get_path()` will return the path to the CSV file along with the total number of records written.
...
```
{
  "file_path": "<path to ./csv_files/export.csv>",
